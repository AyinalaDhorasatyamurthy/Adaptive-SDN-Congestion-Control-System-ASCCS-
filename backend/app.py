...
import json
import joblib
import requests  # ✅ Added for rerouting trigger
...
class SDNMonitor:
    ...
    def predict_congestion(self, stats):
        X = [[stats["backlog"], stats["drops"], stats["overlimits"]]]
    ...
    def fetch_queue_stats(self):
        ...
        # Predict congestion
        congestion = self.predict_congestion(stats)
        stats["congestion_predicted"] = congestion

        # Log to CSV for training data collection
        csv_file = "queue_stats.csv"
        fieldnames = ["timestamp", "backlog", "drops", "overlimits", "congestion"]
        ...
        # ✅ Rerouting logic added
        if congestion == 1:
            try:
                response = requests.post(f"http://{self.vm101_ip}:8080/reroute", timeout=2)
                print("🔁 Rerouting triggered:", response.status_code)
            except Exception as e:
                print("⚠️ Failed to call reroute API:", str(e))
    ...
    def parse_traffic_stats(self, output):
        m = re.search(r'(\d+(?:\.\d+)?)\s+(K|M|G)bits/sec', output)
        if not m:
        ...
        if unit == "K":
            ...
            value *= 1000
        return {"bandwidth": round(value, 2)}
    ...
    def collect_dpi_data(self):
        ...
        if start == -1 or end == -1 or end <= start:
            return {"status": "error", "message": "Failed to locate JSON", "raw_output": raw}

        json_str = raw[start:end+1]
        try:
            parsed = json.loads(json_str)
        except json.JSONDecodeError as e:
            return {"status": "error", "message": str(e), "raw_json": json_str}
